...
When we cut the tree we got four clusters of size 204, 194, 171, and 155. If you
examine the `cutree` documentation (reminder: use `?cutree` from the command
...
cat(cluster3.genes, quote=F,sep="\n")
```
...
The `kmeans` function calculates standard k-means clusters in R. The
input is a data matrix (perhaps transformed before hand) or distance matrix and
$k$, the number of clusters. Alternatively you can specify starting cluster
centers. You can also run the algorithm multiple times with different random
starting positions by using the `nstart` argument.

```{r}
spellman.kmeans <- kmeans(1 -spellman.cor, 8) # create k-means clustering with 8 clusters
kclusters <- spellman.kmeans$cluster
table(kclusters)
```

For comparison with our earlier hierarchical clustering results, lets plot the k-means inferred clusters back onto our dendrogram.
...
Comparing the inferred k-means clustering to our previous complete linkage
...
## Heat map from k-means cluster

In the same manner we generated a heat map for one of the hierarchical clustering sub-trees, we can generate a similar heat map for a k-means cluster. Let's examine cluster 4 which had 62 genes:
...
spellman.long %>%
  ...
  ggplot(aes(x = time, y = gene)) + 
    ...
    scale_fill_gradientn(colors=color.scheme) + 
    labs(title = "K-means Clustering of Spellman et al. Data\nCluster 4") + 
    theme(axis.text.y = element_text(size = 6))  # set size of y axis labels
```
